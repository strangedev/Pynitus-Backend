...
from src.Utilities import BatchProcessing
from src.Utilities.TypingFixes import Maybe, One
...
def __naiveTypeCast(tag_name: str, tag_value: Any) -> TagSupport.TagValue:
    primitive_type = TagSupport.getPrimitiveType(tag_name)
...
def __convertTagType(tag_name: str, tag_value: Any) -> TagSupport.TagValue:
    ...
    if TagSupport.isListType(tag_name):  # Should this attribute be represented as a list?
        ...
        tag_value = BatchProcessing.justList([__naiveTypeCast(tag_name, v) for v in tag_value])
...
def __sanitizeTagValue(tag_name: str, tag_value: Any) -> TagSupport.TagValue:
    sanitization_method = __getSanitizationMethod(TagSupport.getPrimitiveType(tag_name))

    def bound_sanitization_method(v: Any) -> Callable[[One], Maybe(One)]:
        return sanitization_method(tag_name, v)

    sanitized_value = BatchProcessing.apply(bound_sanitization_method, tag_value)

    if type(sanitized_value) is list:
        sanitized_value = BatchProcessing.justList(sanitized_value)

    return sanitized_value


def __sanitizeTagName(tag_name: str) -> str:
    if tag_name not in TagSupport.INTERNAL_NAMES:
        return TagSupport.getInternalName(tag_name)
    return tag_name


def sanitizeTag(tag_name: str, tag_value: Any) -> TagSupport.TagValue:
    """
...
def sanitizeTags(tags: Dict[str, Any]) -> Dict[str, TagSupport.TagValue]:
    """
