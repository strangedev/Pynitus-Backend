...
class HTMLBuilder(object):
    ...
    def buildArtistPage(self, ipAddr, floodProtection, voteHandler, playbackQueue, musicLibrary, artist):
        ...
        return template.render(
            ...
            playing=playbackQueue.playing,
            albumTitles=sorted(
                [album.title for album in musicLibrary.getAlbumsForArtist(artist)]),
            trackAndAlbumTitles=sorted(
                [(track.title, track.albumTitle) for track in musicLibrary.getTracksForArtist(artist)]),
            artistName=artist
        )
    ...
    def buildAddPage(self, ipAddr, floodProtection, voteHandler, playbackQueue, trackfactory):
        ...
        return template.render(
            ...
            votesRequired=voteHandler.getRequiredVotes(),
            playing=playbackQueue.playing,
            trackTypesAndDescs=sorted([(trackType, trackfactory.availableTrackTypes[
